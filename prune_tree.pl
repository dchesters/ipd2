...
#	
#	if user provides fasta file, the IDs are retained and any others pruned.
#	if user provides list on command line or in file, these are pruned.
#	
#	
#	
#	
...
print "phylogeny read from file $treefile, terminal count $#backbone_tree_terminals\n";
...
if($prune_string =~ /\w/)
	{
	print "prune things listed on command line\n";
	...
	}elsif($prune_filename =~ /\w/)
	{
	open(PRUNE_FILE, $prune_filename) || die "\nerror, user specified to prune terminals listed in file $prune_list_file, but cant open.\n";
...
#	print "\nNEW NODE($node)\n";
	...
	for $i(0 .. $#child_nodes)
		...
		unless($child_nodes[$i] =~ /INTERNAL_NODE_/)
			{
			if($terminals{$child_nodes[$i]} == 1){print "\nWARNING, you have duplicated terminal ID (after any prescribed processing):$child_nodes[$i]\n"};
...
if($arguments =~ /-seqfile\s+(\S+)/) # only used to get taxa for pruning, not needed if these are provided on command line
	...
	{
	$prune_filename = $1;
	print "will prune from file $prune_filename\n"; 
	}else{
	unless($arguments =~ /\-start_prune_list\s+(.+)/){die "\ninsufficient input\n"};
	}
...
#	unless($count_decendents_of_parent == 1){die "\nnot written for non-totally bifurcating. (\$count_decendents_of_parent == $count_decendents_of_parent)\n"}
	...
	for my $i(0 .. $children_of_grandparent)
		...
		my $child  = $nodes{$grandparent}{$i}; # print "\tchild of grandparent:$grandparent i:$i is:$child\n";
		if($child eq $parent)	
			{
			...
			for my $j(0 .. $count_decendents_of_parent)
				...
				if($child2 eq $tax)
					...
					# branchlength of node sister to that pruned is
					# its branchlength + that of parent
					 $nodes{$child2}{branchlength} = $nodes{$child2}{branchlength}+$nodes{$parent}{branchlength};
