...
# 
# -input XX -output XX -node XX -fasta or -newick or -list
# 	newick reading disc
# 
# 
# 
...
# 2020-02-15: takes simple list as input. prints species also to tax heirachy.
# 2023-12-05: should now read all terminals from polytomies
# 
# 
...
$tax_table_rank = 1; # 1 = species, 2 = genus
...
if($assign_to_tree==1) #	if $arguments = -newick then $assign_to_tree=1; elsif $arguments = -list then $assign_to_tree = 2
	{
	open(LOG7, ">taxonomic_report_verboseLOG");
...
print "parsing newick string ....\n";
	...
	while ($newick_string =~ /\(([^\(\)]+)\)/) # rooted bipart raxml tree, the 2 last nodes will have no boot values
		...
		print LOG7 "newick_string:$newick_string\n";
		...
		if(scalar @child_nodes == 1)# an error, internal node with no split
			...
			print "error 177, .... length_newick_string:$length_newick_string ID:$nodeID\n";
			if($length_newick_string <= 500){print "\tnode:$node newick_string:$newick_string\n";};
			$interal_node++;
			$root_node = $nodeID;
			};
		...
		#	print "bifurcation $#child_nodes\n";
			...
			for $i(0 .. $#child_nodes)
				...
				unless($child_nodes[$i] =~ /INTERNAL_NODE_/)
					{
					$terminals{$child_nodes[$i]} =1;$count_the_terminal_nodes++; # print "$child_nodes[$i] ";
					}
				};
		...
		#####################################################################################
			...
			while($node =~ s/([^\,]+)\,([^\,]+)/INTERNAL_NODE_$interal_node/)
				...
				unless($member1 =~ /INTERNAL_NODE_/){$terminals{$member1} =1;$count_the_terminal_nodes++};
				unless($member2 =~ /INTERNAL_NODE_/){$terminals{$member2} =1;$count_the_terminal_nodes++};
...
foreach my $tax(@terminal_labels){print TEMPFILE ">$tax\n"};
close TEMPFILE;
close LOG7;

	}elsif($assign_to_tree == 2) #	if $arguments = -newick then $assign_to_tree=1; elsif $arguments = -list then $assign_to_tree = 2
	{

	open(INTREE, $fas_file) || die "\nerror ... input file you have given ($fas_file) cannot be opened. \n";
	while (my $line = <INTREE>)
		{
		$line =~ s/\n//;$line =~ s/\r//;
		if($line =~ /([\S\s]+)/) 
			{
			my $thingy = $1;
			push @terminal_labels , $thingy; $parsedcount++;
			};
		};
	close(INTREE);


unless($parsedcount >= 2){die "\nerror parsing \n"};
...
close TEMPFILE;
 ...
 #  hlj 425; hrb 167; bj 372; yn 162, km 48
...
my @all_fasta_genera = keys %query_genera; @all_fasta_genera = sort @all_fasta_genera;
...
if($tax_table_rank == 1)# 1==species
	{@taxlist = @all_fasta_members}else{@taxlist = @all_fasta_genera};



foreach my $id(@taxlist)
	{
#	print "\n$id\n";
	my $printstring = "$id\t";
...
#	print "id:$id\n";
	...
	my $genus = $id;#print "id:$id\n";
		...
		$count_diet_IDs++;
		$query_IDs{$id}= 1;$query_genera{$genus}= 1;

		unless($species_for_this_genus{$genus} =~ /$binomial\n/){$species_for_this_genus{$genus} .= "$binomial\n"};
		...
		#	print "completelineage:$completelineage\n";
			...
			while($completelineage =~ s/(\S+):(\S+)//)
					...
					$higher_tax_of_member{$genus}{$rank} = $tax;
...
foreach my $child(@child_nodes_array)
	...
	unless($current_node == $child)# one the child nodes of the root node (1), is also 1 
		...
		if($tax_count{$name_string}>=1)
			...
			if($rank eq "genus" && $species_for_this_genus{$name_string} =~ /./){print OUT "$species_for_this_genus{$name_string}"};
